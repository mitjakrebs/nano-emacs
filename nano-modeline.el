...
(defun nano-modeline-compose (status name primary secondary)
  ...
  (let* ((char-width    (window-font-width nil 'header-line))
         ...
         (space-up       +0.15)
         (space-down     -0.20)
	 (prefix (cond ((string= status "r")
			        (propertize (if (window-dedicated-p)" -- " " r ")
                                'face 'nano-face-header-popout))
                   ((string= status "*")
			        (propertize (if (window-dedicated-p)" -- " " * ")
                                'face 'nano-face-header-critical))
                   ((string= status "w")
			        (propertize (if (window-dedicated-p)" -- " " w ")
...
(with-eval-after-load 'ein
  (defun nano-modeline-ein-notebook-mode ()
    (let ((buffer-name (format-mode-line "%b")))
      (nano-modeline-compose (if (ein:notebook-modified-p) "*" "w")
                             buffer-name
                             ""
                             (ein:header-line))))
...
(defun nano-modeline-status ()
  "Return buffer status: read-only (r), modified (*) or read-write (w)"
  
  (let ((read-only   buffer-read-only)
        (modified    (and buffer-file-name (buffer-modified-p))))
    (cond (modified  "*") (read-only "r") (t "w"))))
